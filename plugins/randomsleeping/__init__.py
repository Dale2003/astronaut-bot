...
excel_file = pd.ExcelFile(os.path.join(current_file_dir, 'sleeping.xlsx'))
...
def random_meal():
    ...
    res = '建议您去'+ str(canteen) + '的' + str(dish) + '睡觉捏~'
    return res

guess = on_command("去哪睡觉", block=True, permission=GROUP_MEMBER|PRIVATE|GROUP)
