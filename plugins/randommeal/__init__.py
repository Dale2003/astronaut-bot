...
excel_file = pd.ExcelFile(os.path.join(current_file_dir, 'meal.xlsx'))
...
def random_meal():
    ...
    res = '建议您去'+ str(canteen) + '吃' + str(dish) + '捏~'
    return res

guess = on_command("吃什么", block=True, permission=GROUP_MEMBER|PRIVATE|GROUP)
